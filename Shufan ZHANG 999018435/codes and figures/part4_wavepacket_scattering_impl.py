...
class Scenario:
    ...
    def alpha(self) -> float:
        # The lecture coefficient a0 gives alpha=(a0/hbar)^2.
        return (self.a0 / HBAR) ** 2

    @property
    def sigma_p_amplitude(self) -> float:
        # Standard deviation of the amplitude convention used by gaussian_p.
    ...
    def carrier_wavelength(self) -> float:
        return 2.0 * np.pi * HBAR / self.p0


def saved_part3_peaks() -> dict[str, float]:
...
def basis_consistent_peak(lo: float, hi: float, dx: float = PART4_BASIS_DX) -> float:
    """Maximize T using the same finite-difference basis used for the packet."""
...
def scenario_catalog() -> dict[str, Scenario]:
    part3_peaks = saved_part3_peaks()
    first_peak = part3_peaks["E1"]
    second_peak = part3_peaks["E2"]
    return {
        "blocked": Scenario(
...
def setup(scenario: Scenario, x: np.ndarray, n_p: int):
    if n_p % 2 == 0:
        n_p += 1
    p = np.linspace(scenario.p0 - 6.5 * scenario.a0,
                    scenario.p0 + 6.5 * scenario.a0, n_p)
    ...
    momentum_norm = float(np.sum(np.abs(coeff) ** 2 * weights))
    coeff = coeff / np.sqrt(momentum_norm)
    energy = p**2 / (2.0 * MASS)
    return p, energy, states, transmission, reflection, coeff, weights


def evolve(states: np.ndarray, energy: np.ndarray, coeff: np.ndarray,
           weights: np.ndarray, time: float) -> np.ndarray:
    phase = np.exp(-1j * energy * time / HBAR)
    return (weights * coeff * phase) @ states


def carrier_is_resolved(scenario: Scenario, x: np.ndarray) -> bool:
    if x.size < 2:
        return False
    return float(np.max(np.diff(x))) <= scenario.carrier_wavelength / 4.0


def plot_wave(ax, x: np.ndarray, psi: np.ndarray, time: float,
              scenario: Scenario, *, envelope_mode: bool) -> None:
    if envelope_mode:
        envelope = np.abs(psi)
        ax.plot(x, envelope, color="red", lw=0.8,
                label=r"$+|\Psi|$ envelope")
        ax.plot(x, -envelope, color="blue", ls="--", lw=0.8,
                label=r"$-|\Psi|$ envelope")
    else:
        ax.plot(x, psi.real, color="red", lw=0.8, label=r"Re $\Psi$")
        ax.plot(x, psi.imag, color="blue", ls="--", lw=0.8,
                label=r"Im $\Psi$")
    density_color = "green" if scenario.name == "first_resonance" else "black"
    ax.plot(x, np.abs(psi) ** 2, color=density_color, lw=1.0,
            label=r"$|\Psi|^2$")
...
def energy_display_digits(scenario: Scenario) -> int:
    return 6


def width_display_digits(scenario: Scenario) -> int:
    return 9 if scenario.name == "first_resonance" else 6
...
def summary(scenario: Scenario, n_p: int) -> dict:
    ensure_dir(OUT)
    ...
    wave_axes = [
        fig.add_subplot(packet_grid[row, 0])
        for row in range(len(scenario.snapshots))
    ]
    ax_t.plot(e_scan, t_scan, color="black")
    display_digits = energy_display_digits(scenario)
    energy_markers = (
        (scenario.Emin, "blue", r"$E_{min}$"),
        (scenario.E0, "red", r"Resonance $E_0$"),
        (scenario.Emax, "green", r"$E_{max}$"),
    )
    for value, color, symbol in energy_markers:
        ax_t.axvline(
            value, color=color, ls="--",
            label=rf"{symbol} = {value:.{display_digits}f}",
        )
    ax_t.set(
        xlabel="Energy", ylabel=r"$|T(E)|^2$", ylim=(-0.02, 1.05),
    ...
    for momentum, marker_energy, color, symbol in momentum_markers:
        ...
        ax_p.scatter(
            [momentum], [height], color=color, s=35,
            label=(rf"${symbol}$, E = {marker_energy:.{display_digits}f}, "
                   rf"$|T|^2$ = {marker_t:.4f}"),
        )
    ax_p.set(
        xlabel="Momentum", ylabel=r"$|\Phi(p)|^2$", title=parameter_title(scenario)
    )
    if scenario.name == "first_resonance":
    ...
    carrier_resolved = carrier_is_resolved(scenario, x)
    envelope_mode = False
    spatial_norms: dict[str, float] = {}
    for row, (ax, time) in enumerate(zip(wave_axes, scenario.snapshots)):
        psi = evolve(states, energy, coeff, weights, time)
        plot_wave(ax, x, psi, time, scenario, envelope_mode=envelope_mode)
        spatial_norms[f"t={time:g}"] = float(INTEGRATE(np.abs(psi) ** 2, x))
        if row == 0:
            ax.legend(loc="upper right", fontsize=7)
    wave_axes[-1].set_xlabel("x")
    if scenario.name == "first_resonance":
        fig.suptitle(
            rf"$E_0={scenario.E0:.6f},\ a_0={scenario.a0:.8f}$",
            fontsize=14,
        )
    fig.subplots_adjust(left=0.06, right=0.98, bottom=0.07, top=0.92,
                        wspace=0.25, hspace=0.45)
    figure_path = OUT / f"part4_{scenario.name}.png"
    fig.savefig(figure_path, dpi=150)
    plt.close(fig)
    center_index = int(np.argmin(np.abs(energy - scenario.E0)))
    part3 = saved_part3_peaks()
    dx_global = float(x[1] - x[0])
    diagnostics = {
        "scenario": scenario.name,
        "continuum_packet_only": True,
        "energy_center_source": (
            "Part 3 high-resolution E1 (dx=0.0025)"
            if scenario.name == "first_resonance"
            else "Part 3 high-resolution E2 (dx=0.0025)"
            if scenario.name == "second_resonance"
            else "fixed report scenario"
            if scenario.name == "partially_blocked"
            else "lecture scenario"
        ),
        "energy": {"Emin": scenario.Emin, "E0": scenario.E0, "Emax": scenario.Emax},
        "marker": {
            "sample_energy": float(energy[center_index]),
            "T": float(np.abs(transmission[center_index]) ** 2),
            "R": float(np.abs(reflection[center_index]) ** 2),
            "unitarity_error": float(abs(np.abs(transmission[center_index]) ** 2
                                          + np.abs(reflection[center_index]) ** 2 - 1.0)),
        },
        "part3_high_resolution_reference": part3,
        "basis": {"dx": scenario.basis_dx, "momentum_points": int(p.size)},
        "packet": {
            "a0": scenario.a0,
            "sigma_p_amplitude": scenario.sigma_p_amplitude,
            "momentum_norm": float(np.sum(np.abs(coeff) ** 2 * weights)),
            "integrated_PT": p_trans,
            "integrated_PR": p_refl,
            "integrated_unitarity_error": abs(p_trans + p_refl - 1.0),
            "finite_window_spatial_norms": spatial_norms,
        },
        "basis_unitarity": {
            "maximum_error": float(np.max(np.abs(np.abs(transmission) ** 2
                                                   + np.abs(reflection) ** 2 - 1.0))),
        },
        "global_rendering": {
            "xlim": scenario.xlim,
            "points": int(x.size),
            "dx": dx_global,
            "carrier_wavelength": scenario.carrier_wavelength,
            "samples_per_carrier_wavelength": scenario.carrier_wavelength / dx_global,
            "carrier_numerically_resolved": carrier_resolved,
            "mode": (
                "resolved complex carrier"
                if carrier_resolved
                else "teacher-style Re/Im overview on the full spatial range"
            ),
        },
        "report_layout": {
            "snapshots": scenario.snapshots,
            "spectrum_xlim": scenario.spectrum_xlim,
            "figure_pixels": [2400, 1350],
            "potential_display": "V(x), unscaled",
        },
    }
    diagnostic_path = OUT / f"part4_{scenario.name}_diagnostics.json"
...
def animation(scenario: Scenario, n_p: int, quick: bool, *, local: bool = False) -> Path:
    ...
    if envelope_mode:
        envelope = np.abs(psi0)
        line_real, = ax.plot(x, envelope, color="red", lw=0.8,
                             label=r"$+|\Psi|$ envelope")
        line_imag, = ax.plot(x, -envelope, color="blue", ls="--", lw=0.8,
                             label=r"$-|\Psi|$ envelope")
    else:
        line_real, = ax.plot(x, psi0.real, color="red", lw=0.8,
                             label=r"Re $\Psi$")
        line_imag, = ax.plot(x, psi0.imag, color="blue", ls="--", lw=0.8,
                             label=r"Im $\Psi$")
    density_color = "green" if scenario.name == "first_resonance" else "black"
    line_density, = ax.plot(x, np.abs(psi0) ** 2, color=density_color, lw=1.0,
                            label=r"$|\Psi|^2$")
    potential_line, = ax.plot(x, model_potential(x), color="0.65", lw=0.8,
                              label=r"$V(x)$")
    ax.axhline(0.0, color="0.4", lw=0.5)
    ...
    def update(frame: int):
        ...
        if envelope_mode:
            envelope = np.abs(psi)
            line_real.set_ydata(envelope)
            line_imag.set_ydata(-envelope)
        else:
...
def main() -> None:
    ...
    parser.add_argument("--skip-gifs", action="store_true", help="generate static figures and diagnostics only")
    ...
    for name in selected:
        ...
        diagnostics = summary(scenario, n_p)
        packet = diagnostics["packet"]
        print(
            f"{name}: E0={scenario.E0:.{scenario.energy_digits}f}, "
            f"PT={packet['integrated_PT']:.9f}, PR={packet['integrated_PR']:.9f}"
        )
