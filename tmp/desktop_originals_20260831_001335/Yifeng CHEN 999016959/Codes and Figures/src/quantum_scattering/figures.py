"""Generate the twenty reference-style PNG/GIF results."""
...
def generate_part3(root: Path) -> list[Path]:
    ...
    data_path = output / "transmission_profile.csv"
    with data_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["energy", "transmission", "reflection", "unitarity_error"])
        for values in zip(
            overview.energy,
            overview.transmission,
            overview.reflection,
            overview.unitarity_error,
            strict=True,
        ):
...
def _save_packet_gif(
    ...
    *,
    ...
    potential = double_barrier_potential(x)
    psi = evaluate_packet(state_matrix, basis, coefficients, float(times[0]))
    real_line, = ax.plot(x, psi.real, color="red", lw=0.9, label=r"Re $\Psi$")
    imag_line, = ax.plot(x, psi.imag, color="blue", ls="--", lw=0.9, label=r"Im $\Psi$")
    density_line, = ax.plot(x, np.abs(psi) ** 2, color="black", lw=1.1, label=r"$|\Psi|^2$")
    ...
    def update(time: float) -> tuple[plt.Artist, ...]:
        current = evaluate_packet(state_matrix, basis, coefficients, float(time))
        real_line.set_ydata(current.real)
        imag_line.set_ydata(current.imag)
        density_line.set_ydata(np.abs(current) ** 2)
        ax.set_title(parameter_title + f"\nt={time:.1f}")
        return real_line, imag_line, density_line, potential_line
    ...
    for case in reference_cases(peaks.second.energy):
        ...
        diagnostics = {
            ...
            "energy_center_source": (
                "part3_finite_difference_second_peak"
        ...
        if include_gifs:
            if case.slug == "second_resonance":
                ...
                zoom_times = np.linspace(2500.0, 4300.0, second_frames)
